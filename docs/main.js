...
function changeArrSection(arrChanged) {
	...
	if (arranger == "modx") {
		if (sectionChange == 0) {
			...
			if (arrChanged) setTimeout(() => output.sendControlChange (92, 16, 4), 2000); 
		}
		if (sectionChange == 1) {
			output.sendControlChange (92, 64, 4); 	
			if (arrChanged) setTimeout(() => output.sendControlChange (92, 48, 4), 2000);  			
		}
		if (sectionChange == 2) {
			output.sendControlChange (92, 96, 4); 			
			if (arrChanged) setTimeout(() => output.sendControlChange (92, 80, 4), 2000);  
		}		
		if (sectionChange == 3) {
			output.sendControlChange (92, 96, 4); 			
			if (arrChanged) setTimeout(() => output.sendControlChange (92, 80, 4), 2000); 
